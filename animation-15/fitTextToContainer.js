function fitTextToContainer(text, fontFace, containerWidth) {
    ...
    let canvas = createHiDPICanvas(containerWidth, 0), context = canvas.getContext("2d"), longestLine = getLongestLine(split(text)), fittedFontSize = getFittedFontSize(longestLine, fontFace);
    ...
    function getPixelRatio() {
        let ctx = document.createElement("canvas").getContext("2d"), dpr = window.devicePixelRatio || 1, bsr = ctx.webkitBackingStorePixelRatio || ctx.mozBackingStorePixelRatio || ctx.msBackingStorePixelRatio || ctx.oBackingStorePixelRatio || ctx.backingStorePixelRatio || 1;
        return dpr / bsr;
    }
    ...
    function getFittedFontSize(text, fontFace) {
        ...
        const font = (size, face)=>"".concat(size, "px ").concat(face);
